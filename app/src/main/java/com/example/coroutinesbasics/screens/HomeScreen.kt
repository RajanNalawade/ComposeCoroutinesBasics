...
import com.example.coroutinesbasics.viewmodels.MainUIEvents
...
fun MovieLazyRow(
    ...
    LazyRow(
        ...
        items(
            ...
            ImageRowItem(movie = singleMovie, onItemClicked = onItemClicked)
        }
...
@Composable
fun ImageRowItem(
    movie: Movie, modifier: Modifier = Modifier, onItemClicked: (itemId: Int) -> Unit
) {
    val sizeResolver = rememberConstraintsSizeResolver()
    val painter = rememberAsyncImagePainter(
        model = ImageRequest.Builder(LocalPlatformContext.current).data(movie.backdropUrl)
            .crossfade(true).allowHardware(true).size(sizeResolver).build()
    )
    ...
    fun GreetingPreview() {
        HomeScreen(
            movieUIState = MovieUIState.SuccessMovieResponse(
            response = listOf(
                MovieResponse(
